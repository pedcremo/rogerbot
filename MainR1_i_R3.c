...
/*********** Ajust comportamiento robot per defecte *********/
/***** Variables globals ********/
//Constants Regulador PD 30:1 HP
//uint8_t cont_corba=0;
uint8_t Kp = 18;  //48 lf 18 pots
uint8_t  Kd = 28;//225 lf 28 pots multiplicador 10
volatile uint8_t velocitat = 95; //225 lf 95 pots max 255
		...
		switch(rescue_estat_actual){
				...
				if (strategy=='c'){ //Si estrategia = e ni mirem color ni res que se li parega
					...
					if (ping()<45 ){ //Si el pot esta a menys de 58 mm frenem i llegim color
						...
						if (ping_read>30 && ping_read<45){	
							if (obtenir_errorp()>1){
								move_robot(velocitat/4,velocitat/3,FORWARD,55);
						 	}else if(obtenir_errorp()<-1){
								move_robot(velocitat/3,velocitat/4,FORWARD,55);
						 	}else{					 
							 	move_robot(velocitat/3,velocitat/3,FORWARD,55);
							}
			...
			case 2:				
				...
				if(dir_robot == 1){ //Si el robot va cap a fora del circuit
					...
					//delay_ms(3000);
					move_robot(velocitat,-velocitat,FORWARD,160); //Girem sobre si mateix sentit antihorari uns 60graus
					//delay_ms(3000);					
...
int finding_line(int speedM1, int speedM2, int direction){
	int proporcional = obtenir_errorp();
...
void follow_line_fast(void){
	...
	if (proporcional==9 || proporcional == -9){
		...
		line_following(FORWARD); //Forward
		_delay_ms(1);
	}
}
...
// Interrupcio de temps activada cada milisegon en current_millis emmagatzemem el num de ms
ISR(TIMER1_COMPA_vect)
{
	line_following(FORWARD);
}
void line_following(int direction){ //0 forward,1 backwards
	int errort=0;
