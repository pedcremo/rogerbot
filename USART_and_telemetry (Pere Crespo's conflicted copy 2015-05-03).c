		...
		if(buf->index>0){
				...
				*u8data=buf->buffer[buf->index];*/
				if (buf->buffer[0]=='l'){ //Load Rogerbot settings
					...
					_delay_ms(2);
					buf->index=16;
					buf->buffer[16]='0'+(velocitat/100);
					buf->buffer[15]='0'+((velocitat/10) % 10);
					buf->buffer[14]='0'+(velocitat % 10);
					buf->buffer[13]=',';
					...
					buf->buffer[11]=',';
					buf->buffer[10]='0'+(Kp/100);
					buf->buffer[9]='0'+((Kp/10) % 10);
					buf->buffer[8]='0'+(Kp % 10);

					buf->buffer[7]=',';
					buf->buffer[6]='0'+(Kd/100);
					buf->buffer[5]='0'+((Kd/10) % 10);
					buf->buffer[4]='0'+(Kd % 10);

					buf->buffer[3]=',';
					buf->buffer[2]=strategy;
					...
					if (sharp < 1000){
						buf->buffer[5]='0';
						buf->buffer[4]='0'+(sharp/100);
						buf->buffer[3]='0'+((sharp/10) % 10);
						buf->buffer[2]='0'+(sharp % 10);
					}else{
				...
				/* }else if(buf->buffer[0]=='e'){ //Read encoder counts 
					...
					buf->index=3;
					int read_sensor=obtenir_errorp();
					if (read_sensor<0){
						buf->buffer[3]='-';
